...
for x in range(empleados.get_length()):
    for y in range(empleados.get_length()-1):
        if empleados.get_item(y)[6][0:14] > empleados.get_item(y+1)[6][0:4]:
            burbuja = empleados.get_item(y)
            'empleados[y] = empleados[y+1]'
            empleados.set_item(empleados.get_item(y+1),y)
            'empleados[y+1] = burbuja'
            empleados.set_item(burbuja,y+1)
            print(empleados)
